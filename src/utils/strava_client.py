...
class StravaClient:
    ...
    def fetch_detailed_activities_batch(self, start_date, batch_size=5):
        """
        Fetches a batch of detailed activities from Strava, starting from the given date.
        ...
        try:
            logging.info(f"Fetching up to {batch_size} activities starting from {start_date}")
            summary_activities = list(self.client.get_activities(after=start_date, limit=batch_size))
